...
- The "Duplicate <kind> …" diagnostic now understands `#ifdef` / `#ifndef`,
  ...
  unflagged when the AND of their guards is provably unsatisfiable (e.g.
  `#ifdef X` vs `#ifndef X`, or `#ifdef [X Y]` vs `#ifndef [X Y]`). Anything
  that could both be compiled in – the `#define`s may come from outside –
  is still flagged.
...
- Hover on a fixed action-block attribute (`continueActionBlock`,
  ...
  dropped (these never take a NAME; only the generic `actionblock` does).
  The glossary syntax hints for these keywords were corrected too.
- Hover rework:
  ...
  - on a reference to a question / variable: name, kind and where it is
    ...
    `javascript` and `css`; `full` keeps everything. The excerpt tolerates a
    single blank line between attribute groups (two end it).
