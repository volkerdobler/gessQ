...
- **Multi-Prefix als Komma-String:** `"prefix": "iAB, initActionBlock"` wird als
  *ein* Prefix `"iAB, initActionBlock"` interpretiert. VS Code unterstützt
...
### 3.5 `package.json`
- `engines.vscode: "^1.99.0"` vs. `@types/vscode: "^1.75.0"` – angleichen
...
Prinzipien: klare Trennung *Aktivierung / Provider / Sprachkern (Parser,
Scope, Index) / Infrastruktur / Daten*. Einheitliche Benennung `*Provider.ts`.
...
## 9. Offene Entscheidungen (später abzustimmen)

1. **Mindest-VS-Code-Version** (`engines.vscode`) – welche Version müssen
   Nutzer:innen mindestens haben? Beeinflusst nutzbare APIs.
2. **Glossar online vs. gebündelt** als Default (Setting `gessq.glossary.source`)
   – gebündelt = offline-fähig, online = immer aktuell.
3. **Linter-Strenge**: „fehlendes `;`“ als Warnung default an oder aus?
4. **Publikation**: nur VS Marketplace oder auch Open VSX?
5. **Handbuch-Quelle**: sauberer Markdown-/HTML-Export des Handbuchs
   beschaffbar, um Hover-Texte inhaltlich zu füllen (statt nur Links)?
6. **GNumQ / weitere Fragetypen**: vollständige, aktuelle Liste der gültigen
   Schlüsselwörter von GESS verfügbar? (Basis für `language.json`.)
