"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const scope_1 = require("../src/scope");
// Minimal fake TextDocument matching the properties used by getScopeAt
function makeDoc(lines) {
    return {
        uri: { toString: () => 'test://doc' },
        lineCount: lines.length,
        version: 1,
        lineAt: (i) => ({ text: lines[i], range: {} }),
    };
}
test('getScopeAt detects line comment', () => {
    const doc = makeDoc(['let a = 1; // comment here']);
    const idxCode = doc.lineAt(0).text.indexOf('let');
    const idxComment = doc.lineAt(0).text.indexOf('//') + 1; // inside comment
    expect((0, scope_1.getScopeAt)(doc, 0, idxCode)).toBe(scope_1.ScopeEnum.normal);
    expect((0, scope_1.getScopeAt)(doc, 0, idxComment)).toBe(scope_1.ScopeEnum.comment);
});
test('getScopeAt detects string and escapes', () => {
    const doc = makeDoc([`const s = "hello \"world\"";`]);
    const idxBefore = doc.lineAt(0).text.indexOf('const');
    const idxInString = doc.lineAt(0).text.indexOf('hello');
    const idxQuote = doc.lineAt(0).text.indexOf('"');
    expect((0, scope_1.getScopeAt)(doc, 0, idxBefore)).toBe(scope_1.ScopeEnum.normal);
    expect((0, scope_1.getScopeAt)(doc, 0, idxInString)).toBe(scope_1.ScopeEnum.string);
    // the closing escaped quote should not end the string
    expect((0, scope_1.getScopeAt)(doc, 0, idxQuote + 1)).toBe(scope_1.ScopeEnum.string);
});
test('getScopeAt detects block comments across lines', () => {
    const doc = makeDoc(['code /* start', 'inside comment */ code']);
    expect((0, scope_1.getScopeAt)(doc, 0, 6)).toBe(scope_1.ScopeEnum.comment); // inside block start
    expect((0, scope_1.getScopeAt)(doc, 1, 2)).toBe(scope_1.ScopeEnum.comment); // still inside block
    const idxAfter = doc.lineAt(1).text.indexOf('code');
    expect((0, scope_1.getScopeAt)(doc, 1, idxAfter)).toBe(scope_1.ScopeEnum.normal);
});
//# sourceMappingURL=scope.test.js.map